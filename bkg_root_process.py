...
* Scan ROOT files under **/home/jtong/lbyl/bkg/backgrounds_interpretation**
  and extract *five* analysis histograms from **five distinct folders**:

      • Nominal/3_LbLeventselection/Pair
      • EG_SCALE_ALL__1up/3_LbLeventselection/Pair
      • EG_SCALE_ALL__1down/3_LbLeventselection/Pair
      • EG_RESOLUTION_ALL__1up/3_LbLeventselection/Pair
      • EG_RESOLUTION_ALL__1down/3_LbLeventselection/Pair

  The same histogram list is read in each folder:

      h_ZMassZoom, h_ZMassFine, h_ZptZoom,
      h_Zrapidity, h_ZAcoZoom, h_ZLeadingPhotonET

* Merge ROOT files belonging to the same physics process:
    ──────────────────────────────────────────────────────────────
    Standard samples  :  lbyl, cep, γγ→ee  
    ALP signal samples:  one sample per mass point (folder “<mass>GeV”)  
    ──────────────────────────────────────────────────────────────

* **Normalise** Monte-Carlo yields so their *signal-region integrals*
  match the expected numbers (see `EXPECTED_COUNTS`).

* **Store every histogram — nominal plus ±1 σ templates — in a nested
  dictionary and pickle it**.  For the signal region the structure is:
  ...
  Control-region histograms are still saved **nominal-only**.

* The resulting pickle **bkg/bkg_alp_sr_pickle.pkl** is consumed by
...
HIST_DIR_KEY = "Nominal/3_LbLeventselection/Pair"
# --- NEW: folders that carry the ±1 σ templates --------------------
SYSTEMATICS = {
    "EG_SCALE_ALL__1up"      : "EG_SCALE_ALL__1up/3_LbLeventselection/Pair",
    "EG_SCALE_ALL__1down"    : "EG_SCALE_ALL__1down/3_LbLeventselection/Pair",
    "EG_RESOLUTION_ALL__1up" : "EG_RESOLUTION_ALL__1up/3_LbLeventselection/Pair",
    "EG_RESOLUTION_ALL__1down": "EG_RESOLUTION_ALL__1down/3_LbLeventselection/Pair",
}

NO_SYST_SAMPLES = {"data"}
...
def extract_hist(root_file, hist_name, folder=HIST_DIR_KEY):
    """Return (counts, bin_edges) from *folder/hist_name* or (None, None)."""
    if root_file is None:
        return None, None

    key = f"{folder}/{hist_name}"
    try:
        h = root_file[key]
...
for sample, file_list in SAMPLES.items():
    ...
    has_syst = sample not in NO_SYST_SAMPLES
    ...
    if has_syst:                                 
        sample_hists_syst = {s: {h: None for h in HIST_NAMES} for s in SYSTEMATICS}
        sample_edges_syst = {s: {h: None for h in HIST_NAMES} for s in SYSTEMATICS}
    ...
    for filename in file_list:
        ...
        for hist_name in HIST_NAMES:
            ...
            # ---- Read ±1 σ variations --------------------------
            if has_syst:
                for syst_key, syst_dir in SYSTEMATICS.items():
                    s_cnt, s_edg = extract_hist(root_file, hist_name, syst_dir)

                    # fall back to zeros when the variation is missing
                    if s_cnt is None:
                        s_cnt = np.zeros_like(counts)
                        s_edg = edges

                    if sample_hists_syst[syst_key][hist_name] is None:
                        sample_hists_syst[syst_key][hist_name] = s_cnt
                        sample_edges_syst[syst_key][hist_name] = s_edg
                    else:
                        sample_hists_syst[syst_key][hist_name] += s_cnt
        ...
        for h in HIST_NAMES:
            ...
            if has_syst:                          
                for syst in SYSTEMATICS:
                    sample_hists_syst[syst][h] *= scale_factor
    ...
    entry = {
        "nominal": {
            h: {
                "bin_edges": sample_edges[h].tolist(),
                "counts":    sample_hists[h].tolist(),
            }
            for h in HIST_NAMES
        }
    }

    if has_syst:                   # ← only add systematics when present
        entry["systematics"] = {
            syst: {
                h: {
                    "bin_edges": sample_edges_syst[syst][h].tolist(),
                    "counts":    sample_hists_syst[syst][h].tolist(),
                }
                for h in HIST_NAMES
            }
            for syst in SYSTEMATICS
