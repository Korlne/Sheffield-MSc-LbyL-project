...
# =============================================================================
#  Light-by-Light (LbyL) photon–pair analysis helpers
#  ==================================================
#
#  What this file does
#  -------------------
#  • Builds a small toolbox around **PyROOT** for selecting diphoton events
#    in ATLAS heavy-ion AODs (√s<sub>NN</sub> ≈ 5.36 TeV).
#  • Applies up-to-date EG-calibration, photon–ID deep-network scores,
#    trigger/scale-factor weights, and a configurable acoplanarity cut.
#  • Returns **plain Python containers** (lists, dicts) so the output can be
#    dumped with `pickle` and analysed later without needing ROOT.
#
#  What the pickled object looks like
#  ----------------------------------
#  The main worker, `process_events`, returns **one dictionary**:
#
#      {
#          "cut_flow"               : {step → int},
#          "diphoton_masses"        : [m<sub>γγ</sub> (GeV)],          # per-event
#          "diphoton_pts"           : [p<sub>T</sub><sup>γγ</sup> (GeV)],
#          "diphoton_acoplanarity"  : [A<sub>φ</sub>],
#          "leading_photon_ets"     : [E<sub>T</sub><sup>lead γ</sup> (GeV)],
#          "diphoton_rapidity_diff" : [Δy],
#          "diphoton_cos_thetas"    : [|cos θ*|],
#          "event_weights"          : [w<sub>L1</sub>·w<sub>HLT</sub>·w<sub>SF</sub>],
#
#          # photon-level collections (one entry **per passing photon**)
#          "photon_pt"              : [p<sub>T</sub><sup>γ</sup> (GeV)],
#          "photon_eta"             : [η<sup>γ</sup>],
#          "photon_idNN_out"        : [raw DNN score],
#          "photon_idNN_pass"       : [bool],
#
#          # running counters for an aco-threshold optimisation scan
#          "aco_scan"               : {"aco<=0.005": N, … "aco<=0.020": N},
#      }
#
#  Every list has the **same length** (N events) except the four
#  `photon_*` arrays, whose size is the total number of *selected photons*
#  in the sample.  Because the structure contains only built-in types,
#  serialising is as easy as
#
#  ```python
#  import pickle
#  with open("my_sample.pkl", "wb") as fout:
...
#  ```python
#  import ROOT, pickle
#  import lbyl_common as lbyl          # <— this file
#
#  # 1) Build a TChain from helper
#  chain = ROOT.TChain("CollectionTree")
#  for f in lbyl.get_sample_file_list("signal"):
#      chain.Add(f)
#
#  # 2) Run the event loop in the *central-region* control region
#  results = lbyl.process_events(chain, aco_mode="cr")
#
#  # 3) Persist results and show cut-flow
#  with open("signal_cr.pkl", "wb") as fout:
#      pickle.dump(results, fout)
#
#  lbyl.print_cutflow("Signal (CR)", results["cut_flow"])
#  ```
#
#  To switch to the **signal region** change `aco_mode="sr"`.  
#  To fill the running **acoplanarity optimisation scan** use
#  `aco_mode="sr_scan"` and read out `results["aco_scan"]`.
#
#  Dependencies
#  ------------
...
def get_sample_file_list(sample):
    ...
    if s == "alp5":
        return [os.path.join(alp_dir, "AOD.41382338._000001.pool.root.1"), # mc23_5p36TeV.860227.SuperChic_4p2_axion_ma5_v2.merge.AOD.e8558_e8528_s4357_s4258_r15716_r15516_tid41382338_00
                ]
    elif s == "alp6":
        ...
        alp_dir = "/home/jtong/lbyl/data/alp/"
        return [
            os.path.join(alp_dir, f)
            for f in os.listdir(alp_dir)
            if f.endswith(".pool.root.1")
        ]
    else:
...
def process_events(t, aco_mode="cr"):
    ...
    # --- acoplanrity scan ---
    ACO_SCAN = {f"aco<={s:.3f}": 0 for s in np.arange(0.005, 0.021, 0.001)}
    ...
    for i in range(total_entries):
        ...
        for ph in t.Photons:
            ...
            f1     = ph.showerShapeValue(ROOT.xAOD.EgammaParameters.f1)
            weta2 = ph.showerShapeValue(ROOT.xAOD.EgammaParameters.weta2)
            eratio = ph.showerShapeValue(ROOT.xAOD.EgammaParameters.Eratio)
            fracs1   = ph.showerShapeValue(ROOT.xAOD.EgammaParameters.fracs1)
            f1core = ph.showerShapeValue(ROOT.xAOD.EgammaParameters.f1core)
        ...
        if aco_mode == "cr":
            ...
            #
            for aco_thr in np.arange(0.005, 0.021, 0.001):
                if A_phi >= aco_thr:
                    continue
...
# =====================================
# Acoplanarity-scan printer
# =====================================
def print_aco_scan(scan_dict, header="Acoplanarity scan"):
    print(f"\n{header}:")
    # keep thresholds numerically sorted
    for key in sorted(scan_dict,
                      key=lambda k: float(k.split('<=')[-1])):
        print(f"{key:10s} : {scan_dict[key]:7d}")