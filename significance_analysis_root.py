...
def parse_cli():
    ...
    # Show ROI and distribution
    p.add_argument(
        "--plot-roi",
        action="store_true",
        help="Plot signal & background histograms with the chosen ROI highlighted",
    )
    return p.parse_args()
...
# Plot signal and bkg distribution + ROI indication
def plot_histograms_with_roi(edges, sig, bkg, lo, hi, label):
    """Quick look: S & B histograms with the ROI shaded."""
    centres = 0.5 * (edges[:-1] + edges[1:])
    plt.figure(figsize=(7, 4.5))
    plt.step(centres, bkg, where="mid", lw=1.4, label="Background", color="tab:grey")
    plt.step(centres, sig, where="mid", lw=1.4, label="Signal",     color="tab:blue")
    plt.axvspan(edges[lo], edges[hi], color="gold", alpha=0.25, label="ROI")
    plt.yscale("log")
    ...
    plt.savefig(out / f"{label}_roi.png", dpi=150)
    plt.close()
...
def process(args, sig_key):
    ...
    # Optional visual sanity-check of the chosen ROI
    if args.plot_roi:
        plot_histograms_with_roi(edges, cnt_sig, cnt_bkg, lo, hi, sig_key)
